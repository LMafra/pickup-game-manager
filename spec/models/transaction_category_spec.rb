...
RSpec.describe TransactionCategory, type: :model do
  ...
  describe 'validations' do
    ...
    it 'requires name' do
      category = TransactionCategory.new(description: 'Test description')
    ...
    it 'enforces name uniqueness' do
      ...
      duplicate_category = TransactionCategory.new(name: existing_category.name, description: 'Different description')
    ...
    it 'does not require description' do
      category = TransactionCategory.new(name: 'Test Category')
  ...
  describe 'scopes' do
    it 'can find categories by name' do
      daily_category = TransactionCategory.find_by(name: 'daily')
      expect(daily_category).to eq(transaction_categories(:daily_transaction))
    end

    it 'can find categories by description' do
      daily_category = TransactionCategory.find_by(description: 'Daily game participation income')
      expect(daily_category).to eq(transaction_categories(:daily_transaction))
    end
  end
  ...
  describe 'model behavior' do
    it 'can create a new transaction category' do
      category = TransactionCategory.new(
        name: 'Weekly',
      ...
      expect(TransactionCategory.count).to eq(3)
    end
